...
this.ngxService.start()
    ...
    this.adminService.getMake().subscribe(
      res => {
        this.makes = res['success'];
        this.totalEntries = this.listUsers.length;
        this.makes.sort((val1, val2)=> {return <any> new Date(val2.created_at) - <any> new 
          Date(val1.created_at)})
        this.ngxService.stop();
      },
      err => { 
  ...
  if (this.signupForm.invalid) {
      ...
      this.adminService.registerModel(formData).subscribe((result) => {
        this.ngxService.stop();
  ...
  updateUser(template:any,userDetails){
    ...
    this.infos.make = userDetails.model;
    
    this.infos.type =  userDetails.make_id;
  ...
  update(){
       ...
       this.adminService.updateModel(formData,this.userId).subscribe((result) => {
        this.router.navigateByUrl('admin/dashboard', {skipLocationChange: true}).then(()=>
        this.router.navigate(["admin/models"]));
        this.ngxService.stop();
      ...
      confirmDelete(){
        this.ngxService.start();
        this.adminService.deleteModel(this.id).subscribe(
          res => {
            this.router.navigateByUrl('admin/dashboard', {skipLocationChange: true}).then(()=>
            this.router.navigate(["admin/models"]));
